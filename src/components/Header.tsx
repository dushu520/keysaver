...
export function Header({ onAddClick, keyCount = 0, onSearch }: HeaderProps) {
  return (
    <header className="flex items-center justify-between px-3 py-2 border-b border-border header-shadow bg-card/50 backdrop-blur-sm gap-4">
      ...
      <button
        onClick={onAddClick}
        className="p-2 primary-btn text-white rounded-lg font-medium flex items-center gap-2 flex-shrink-0"
        title="添加密钥"
      >
